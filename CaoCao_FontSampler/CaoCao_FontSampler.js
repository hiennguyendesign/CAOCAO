...
var counter=0; 
var pangrams=['CaoCao is a bilingual English and Vietnamese display font.','CaoCao là kiểu chữ song ngữ Anh Việt. ','The quick brown fox jumps over the lazy dog.','Do bạch kim rất quý, sẽ để lắp vô xương.','Oh my God!','Trời ơi!','This little piggy went to the market, this little piggy stayed home.','Con sâu lúc lắc anh không dám bắt, con sâu chết ngắt anh bắt cho em coi.','This bowl of phở is only $5.','Tô phở này chỉ có 5₫ thôi.','Hello! How was your day?','Chào! Hôm nay thế nào?']; 
var para = pangrams[counter];
...
function preload() {
  font = loadFont('data/CaoCao.otf');   // this font file must exist
}

function setup() {
  createCanvas(windowWidth, windowHeight);
  background(255);
  textFont('CaoCao', height/7);
  typesize=height/7;
  linesize=typesize;
...
function draw(){
  ...
  fill(0);
  noStroke();
  text('Right Arrow: Toggle Sentence/Full Glyph Set                   Left Arrow: Toggle Stroke                       Down Arrow: Toggle Full/English/Vietnamese Glyph Sets',windowWidth/8,30);
  text('Mũi tên phải: Chuyển đổi Câu/Bộ Glyph đầy đủ               Mũi tên trái: Chuyển đổi Stroke              Mũi tên xuống: Chuyển đổi bộ Glyph đầy đủ/tiếng Anh/tiếng Việt',windowWidth/8,50);
  pop();
...
function keyPressed(){
  if (keyCode === DOWN_ARROW){
    //toggles between full/eng/viet glyph sets
   if (glyphSet=='full') {
      glyphSet='eng';
    } else if (glyphSet=='eng') {
      glyphSet='viet';
   ...
   viewOutline=!viewOutline;
    ...
    //toggles Pangram <-> Full Glyph Set
    frameCount=0;
    viewAlphabet=!viewAlphabet;
    para=pangrams[counter];
   if (viewAlphabet) {
    counter++;
   }
   if (counter>11){
